...
export class NASAPowerService {
  ...
  calculateDateSpecificAverages(data, targetDate) {
    ...
    const getDateSpecificValues = (paramValues) => {
      ...
      for (const [dateStr, value] of Object.entries(paramValues)) {
        if (value !== null && value !== undefined && value !== -999 && !isNaN(value)) {
    ...
    if (parameters.PRECTOTCORR) {
      ...
      if (precipValues.length > 0) {
        ...
        const extremeRainProbability = (extremeRainDays / precipValues.length) * 100;
    ...
    if (parameters.PRECTOTCORR) {
      ...
      if (weeklyPrecipValues.length > 0) {
        const weeklyRainyDays = weeklyPrecipValues.filter(val => val > 0.1).length;
        const weeklyRainProbability = (weeklyRainyDays / weeklyPrecipValues.length) * 100;
  ...
  calculateDroughtProbability(precipData) {
    const dateEntries = Object.entries(precipData).sort(([a], [b]) => a.localeCompare(b));
    let droughtDays = 0;
  ...
  calculateSeasonalRainPatterns(precipData) {
    ...
    const overallAvg = avgValues.reduce((acc, val) => acc + val, 0) / avgValues.length;
  ...
  formatAnnualData(nasaData, lang = 'en') {
    ...
    const dateStr = targetDateObj.toLocaleDateString(isArabic ? 'ar-SA' : 'en-US', {
      month: 'long',
      day: 'numeric'
    });
...
📊 Rain Statistics for This Date Across ${location.yearsOfData} Years:`;
    ...
    if (averages['RAIN_PROBABILITY_TODAY']) {
      ...
      const dryProb = averages['DRY_DAY_PROBABILITY_TODAY']?.average || (100 - rainProb);
      let interpretation = '';
      
      if (rainProb < 5) {
        interpretation = isArabic ? 'يوم جاف تماماً تقريباً' : 'Almost always a dry day';
      } else if (rainProb < 15) {
        interpretation = isArabic ? 'عادة ما يكون يوماً جافاً' : 'Usually a dry day';
      } else if (rainProb < 35) {
        interpretation = isArabic ? 'احتمالية مطر منخفضة' : 'Low chance of rain';
      } else if (rainProb < 65) {
        interpretation = isArabic ? 'احتمالية مطر معتدلة' : 'Moderate chance of rain';
      } else if (rainProb < 85) {
        interpretation = isArabic ? 'احتمالية مطر عالية' : 'High chance of rain';
      } else {
        interpretation = isArabic ? 'عادة ما يكون يوماً ممطراً' : 'Usually a rainy day';
      }
    ...
    const otherParams = Object.keys(averages).filter(param => !rainParams.includes(param));
    if (otherParams.length > 0) {
      formatted += isArabic ? '\n\n🌡️ بيانات مناخية أخرى لهذا التاريخ:' : '\n\n🌡️ Other Climate Data for This Date:';
      
      for (const param of otherParams.slice(0, 4)) { // Show only top 4 to keep it concise
        if (averages[param]) {
          const data = averages[param];
          const name = this.getParameterName(param, lang);
          const avg = data.average.toFixed(1);
