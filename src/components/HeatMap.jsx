...
export default function HeatMap({ variable }) {
  ...
  console.log("HeatMap - globalWeatherData:", globalWeatherData);
  console.log("HeatMap - variable:", variable);

  if (!globalWeatherData || Object.keys(globalWeatherData).length === 0) {
    return (
      <div className="w-full h-[200px] flex items-center justify-center bg-gray-100 rounded-lg">
        <p className="text-gray-500">Loading heat map data...</p>
      </div>
    );
  }
  ...
  console.log("HeatMap - variableData:", variableData);

  if (!variableData || Object.keys(variableData).length === 0) {
    return (
      <div className="w-full h-[200px] flex items-center justify-center bg-gray-100 rounded-lg">
        <p className="text-gray-500">No data available for {variable}</p>
      </div>
    );
  }
